...
class Pen1Style implements Style {
    private float prevX;
    private float prevY;
...

    {
        ...
        pattern.setAlpha(StylesFactory.ALPHA_PEN1);
    }

    @Override
    public void stroke(Canvas c, float x, float y) {
        int size = (int)Utils.getDimention(5);
        size += Surface.thickness * size / 2;
        pattern.setColorFilter(Surface.color, PorterDuff.Mode.SRC_IN);

        float deltaX = x - prevX;
        float deltaY = y - prevY;
        float dist = (float) Math.sqrt(deltaX * deltaX + deltaY * deltaY);
        int cnt = (int) Math.max(dist*3, 1);
        for (int i = 0; i < cnt; i ++) {
